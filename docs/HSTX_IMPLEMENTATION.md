...
- **Core 0**: Handles pixel-perfect video capture (PIO1+DMA). The stable path uses one 32K-entry RGB565 LUT. The optional Colors menu keeps two such LUTs and selects one pointer at input VSYNC, with the same one-read pixel loop. A separate default-off DARK/SHADOW experiment uses an 8.25 KiB split LUT and is timing-regressed on hardware.
