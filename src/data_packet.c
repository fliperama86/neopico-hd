...
void packet_set_audio_infoframe(data_packet_t *packet, uint32_t sample_rate,
    ...
    switch (bits_per_sample) {
        ...
        default: ss = 0x00; break;  // Refer to stream header
    }
    ...
    switch (sample_rate) {
        ...
        default:     sf = 0x00; break;  // Refer to stream header
    }
...
// Channel status: Simplified to match PicoDVI (constant, no dynamic computation)
...
int packet_set_audio_samples(data_packet_t *packet, const audio_sample_t *samples,
    ...
    for (int i = 0; i < num_samples; i++) {
        ...
        // Match PicoDVI exactly: constant vuc=1
        // V=1 (technically "invalid" but receivers accept it)
        // U=0, C=0 (no channel status, receiver sees 44.1kHz)
        const uint8_t vuc_left = 1;
        const uint8_t vuc_right = 1;
...
static void encode_header_to_lane0(const data_packet_t *packet, uint16_t *lane0,
    ...
    int hv1 = hv | 0x08;  // Bit 3 = 1 after first clock
    if (!first_packet) {
        hv = hv1;
    }
    ...
    for (int i = 0; i < 4; i++) {
        ...
        lane0[idx++] = TERC4[((h << 2) & 4) | hv];  hv = hv1;
...
static void encode_subpackets_to_lanes(const data_packet_t *packet,
    ...
    for (int i = 0; i < 8; i++) {
        ...
        uint32_t v = (packet->subpacket[0][i] << 0) |
                     (packet->subpacket[1][i] << 8) |
                     (packet->subpacket[2][i] << 16) |
        ...
        // hsdaoh packs 2 symbols per word: makeTERC4x2Char_2(a, b) = TERC4[a] | (TERC4[b] << 10)
        // Then HSTX conversion unpacks: first output = lower 10 bits, second = upper 10 bits
        // So their symbol order per byte is: (v>>0), (v>>16), (v>>4), (v>>20)
        // And for lane 2: (v>>8), (v>>24), (v>>12), (v>>28)
...
void debug_encode_byte(uint8_t byte_val) {
    ...
    printf("  Lane1 nibbles (0,16,4,20): %lx %lx %lx %lx\n",
           (v >> 0) & 0xF, (v >> 16) & 0xF, (v >> 4) & 0xF, (v >> 20) & 0xF);
    printf("  Lane2 nibbles (8,24,12,28): %lx %lx %lx %lx\n",
           (v >> 8) & 0xF, (v >> 24) & 0xF, (v >> 12) & 0xF, (v >> 28) & 0xF);
}
...
void debug_print_data_island(const hstx_data_island_t *island) {
    ...
    for (int byte_idx = 0; byte_idx < 8; byte_idx++) {
        ...
        printf("    Byte %d (words %d-%d): L1=[%x,%x,%x,%x] L2=[%x,%x,%x,%x]\n",
               byte_idx, base_word, base_word+3,
               lane1_nibbles[0], lane1_nibbles[1], lane1_nibbles[2], lane1_nibbles[3],
               lane2_nibbles[0], lane2_nibbles[1], lane2_nibbles[2], lane2_nibbles[3]);
    }
...
void debug_dump_data_island(const hstx_data_island_t *island) {
    ...
    for (int i = 0; i < 36; i += 4) {
        printf("  [%2d-%2d]: %08lx %08lx %08lx %08lx\n",
               i, i+3,
               island->words[i], island->words[i+1],
               island->words[i+2], island->words[i+3]);
    }
}
...
void debug_hsdaoh_acr_comparison(uint32_t n, uint32_t cts) {
    ...
    // Byte 1 (SB1 = CTS[7:0]):
    uint32_t v = cts_byte0 | (cts_byte0 << 8) | (cts_byte0 << 16) | (cts_byte0 << 24);
    uint32_t t = (v ^ (v >> 7)) & 0x00aa00aa;
    v = v ^ t ^ (t << 7);
    t = (v ^ (v >> 14)) & 0x0000cccc;
    v = v ^ t ^ (t << 14);
    ...
    printf("  HSTX words (lane1 only): [%03x, %03x, %03x, %03x]\n",
           packed01 & 0x3FF, packed01 >> 10,
           packed23 & 0x3FF, packed23 >> 10);
}
...
static void init_null_islands(void) {
    if (null_islands_initialized) return;
    ...
    for (int vsync = 0; vsync < 2; vsync++) {
        for (int hsync = 0; hsync < 2; hsync++) {
            hstx_encode_data_island(&null_islands[vsync * 2 + hsync],
                                     &null_packet, vsync, hsync);
        }
    }
