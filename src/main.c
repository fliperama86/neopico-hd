...
static const int16_t sine_table[128] = {
    0x0000, 0x0648, 0x0C8C, 0x12C8, 0x18F9, 0x1F1A, 0x2528, 0x2B1F, 0x30FC,
    0x36BA, 0x3C57, 0x41CE, 0x471D, 0x4C40, 0x5134, 0x55F6, 0x5A82, 0x5ED7,
    0x62F2, 0x66D0, 0x6A6E, 0x6DCA, 0x70E3, 0x73B6, 0x7642, 0x7885, 0x7A7D,
    0x7C2A, 0x7D8A, 0x7E9D, 0x7F62, 0x7FD9, 0x7FFF, 0x7FD9, 0x7F62, 0x7E9D,
    0x7D8A, 0x7C2A, 0x7A7D, 0x7885, 0x7642, 0x73B6, 0x70E3, 0x6DCA, 0x6A6E,
    0x66D0, 0x62F2, 0x5ED7, 0x5A82, 0x55F6, 0x5134, 0x4C40, 0x471D, 0x41CE,
    0x3C57, 0x36BA, 0x30FC, 0x2B1F, 0x2528, 0x1F1A, 0x18F9, 0x12C8, 0x0C8C,
    0x0648, 0x0000, 0xF9B8, 0xF374, 0xED38, 0xE707, 0xE0E6, 0xDAD8, 0xD4E1,
    0xCF04, 0xC946, 0xC3A9, 0xBE32, 0xB8E3, 0xB3C0, 0xAECC, 0xAA0A, 0xA57E,
    0xA129, 0x9D0E, 0x9930, 0x9592, 0x9236, 0x8F1D, 0x8C4A, 0x89BE, 0x877B,
    0x8583, 0x83D6, 0x8276, 0x8163, 0x809E, 0x8027, 0x8001, 0x8027, 0x809E,
    0x8163, 0x8276, 0x83D6, 0x8583, 0x877B, 0x89BE, 0x8C4A, 0x8F1D, 0x9236,
    0x9592, 0x9930, 0x9D0E, 0xA129, 0xA57E, 0xAA0A, 0xAECC, 0xB3C0, 0xB8E3,
    0xBE32, 0xC3A9, 0xC946, 0xCF04, 0xD4E1, 0xDAD8, 0xE0E6, 0xE707, 0xED38,
    0xF374, 0xF9B8,
};
...
static inline void convert_and_store_pixels(uint32_t word, uint16_t *dst) {
  ...
  // Per pixel: bit 0 = PCLK (ignore), bits 1-5 = G4-G0 (reversed), bits 6-10 =
  // B0-B4, bits 11-15 = R0-R4
...
int main() {
  ...
  mvs_pixel_capture_program_init(pio_mvs, sm_pixel, offset_pixel, PIN_BASE,
                                 PIN_CSYNC, PIN_PCLK);
...
#else
      .pio = pio0, // Fallback for RP2040 (PIO2 only exists on RP2350)
#endif
      .sm = 0,
  };
